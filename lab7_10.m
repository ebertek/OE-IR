...
tic
%keresett fekete-fehér kép:
GA_TARGET = double(imresize(im2bw(imread('lab5_happy02.png'),0.5),[64 64]));
GA_POP_SIZE=1000; %generáció méret
GA_MAX_ITER=500; %max iteráció
GA_ELITE_RATE=0.1; %elitráta
GA_MUTATION_RATE=0.0005; %mutációs ráta
...
while Generation<GA_MAX_ITER
    ...
    %keresztezés
    for i=1:GA_POP_SIZE
        Parent1=randi([1 floor(GA_ELITE_RATE * GA_POP_SIZE)]); %egyik szülõ
        Parent2=randi([1 floor(GA_ELITE_RATE * GA_POP_SIZE)]); %másik szülõ
        CrossoverPoint=randi([0 1],length(GA_TARGET),length(GA_TARGET)); %véletlen keresztezés
        Population(:,:,i)=EliteSelection(:,:,Parent1).*CrossoverPoint + EliteSelection(:,:,Parent2).*(1-CrossoverPoint);
    end
    %véletlen mutáció, gén=pixel
